...
def dim_eight_pixels_image():
    ...
    for row in range(im.height):
        for col in range(im.width):
            ...
            r_dim = int(r / 2)
            g_dim = int(g / 2) #issues with .putpixel not liking floats so just forced value to int
            b_dim = int(b / 2)
            im_dim.putpixel((col, row), (r_dim, g_dim, b_dim))
...
def blue_washed_image():
    ...
    for col in range(im.width):
        for row in range(im.height):
            ...
            im_blue.putpixel((col, row), (new_r, new_g, new_b))
