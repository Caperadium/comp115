...
def remove_duplicates_set(s):
    ...
    Returns:
    result: A string containing unique characters from the input string.
    """
    seen_set = set()
    result = ''
    for c in s:
...
def gem_counting(stones, gems):
    """
    Counts number of gems in string stones

    Parameters: str: stones, str:gems

    Returns: gem_count, number of characters in both stones and gems
    """
...
def students_id(ids):
    """
    Calculate the number of different students who use the shuttle service.

    Parameters:
    ids (list): A list of student IDs.

    Returns:
    len(student_id_set): The number of different students who use the shuttle service.
    """
...
def students_id_occurrences(ids):
    """
    Count the occurrences of each different student who uses the shuttle service.

    Parameters:
    ids (list): A list of student IDs.

    Returns:
    id_dict: A dictionary containing the occurrences of each student.
    """
    id_dict = {}

    for student in ids:
        count = ids.count(student)
        id_dict[student] = count
    return(id_dict)
...
def word_frequency(paragraph):
    """
    Count the frequency of words in a given paragraph.

    Parameters:
    paragraph (str): A string representing a paragraph.

    Returns:
    word_dict: A dictionary containing the frequency of each word.
    """
    word_dict = {}
    split_paragraph = paragraph.split(' ')

    for word in split_paragraph:
        count = split_paragraph.count(word)
